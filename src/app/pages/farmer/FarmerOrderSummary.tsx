...
import { ArrowLeft, Calendar, Package, CheckCircle } from 'lucide-react';
import { useFreshProduce } from '../../contexts/FreshProduceContext';

export function FarmerOrderSummary() {
  const navigate = useNavigate();
  const { farmerProducts, periods, markFarmerReady, getFarmerOrderSummary } =
    useFreshProduce();
  ...
  // Get farmer's products for this period
  const myProducts = farmerProducts.filter(
    (p) => p.farmerId === farmerId && p.periodId === currentPeriod.id
  );

  // Mock: Calculate what customers ordered (in real app, this comes from actual orders)
  // For demo, we'll show 70% of uploaded stock as ordered
  const orderedProducts = myProducts.map((p) => ({
    productName: p.productName,
    quantityOrdered: Math.floor(p.quantity * 0.7),
    unit: p.unit,
  }));
            ...
            >
              <div className="mb-4">
                <span className="text-4xl">
                  {item.productName === 'Tomate'
                    ? '🍅'
                    : item.productName === 'Brócoli'
                    ? '🥦'
                    : item.productName === 'Espinaca'
                    ? '🥬'
                    : '🌿'}
                </span>
              </div>
            ...
            <div>
              ...
              <p className="text-base text-[#757575] leading-relaxed">
                ...
                <strong className="text-[#1C1C1C]">
                  {new Date(currentPeriod.endDate.getTime() + 2 * 24 * 60 * 60 * 1000).toLocaleDateString(
                    'es-PE',
